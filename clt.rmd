...
## Rolling five dices, calculate the sample mean for 100000 times

```{r fig.width=7, fig.height=6}
fivedices <- replicate(100000, mean(sample(c(1,2,3,4,5,6), 5, replace=TRUE)))
hist(fivedices, main="Mean score of 5 dices, repeat for 100000 times", xlab="Score", ylab="Freq", xlim=c(0,7))
```

The mean and SD are `r mean(fivedices)` and `r sd(fivedices)` respectively.

## Rolling 100 dices, calculate the sample mean for 100000 times

```{r fig.width=7, fig.height=6}
hundreddices <- replicate(100000, mean(sample(c(1,2,3,4,5,6), 100, replace=TRUE)))
hist(hundreddices, main="Mean score of 100 dices, repeat for 100000 times", xlab="Score", ylab="Freq", xlim=c(0,7))
```
...
library(plyr)
kable(ldply(data.frame(onedice, fivedices,hundreddices),function(x) { c(mean(x), sd(x)) }))
```
[dice]: dice01.jpg